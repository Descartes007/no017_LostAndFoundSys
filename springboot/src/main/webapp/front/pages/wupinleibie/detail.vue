...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','wupinleibie');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','wupinleibie');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('wupinleibie', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusswupinleibie/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('wupinleibie', this.detail);
				this.$utils.msg('审核成功');
