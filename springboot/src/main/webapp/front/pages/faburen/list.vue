<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<input v-model="searchForm.faburenzhanghao" type="text" placeholder="发布人账号" ></input>
			</view>
                  			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.faburenxingming" type="text" placeholder="发布人姓名" ></input>
			</view>
        ...
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item" :style='{"padding":"20rpx 30rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"14rpx 0","borderColor":"rgba(22, 113, 193, 1)","backgroundColor":"rgba(255, 255, 255, 1)","borderRadius":"60rpx 0 60rpx 0","borderWidth":"4rpx","width":"100%","borderStyle":"solid","height":"auto"}' style="display: flex;">
            ...
            <text v-if="userid && isAuth('faburen','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('faburen','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('faburen','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('faburen','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.faburenzhanghao="";
				this.searchForm.faburenxingming="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.faburenzhanghao){
					params['faburenzhanghao'] = '%' + this.searchForm.faburenzhanghao + '%'
				}
				if(this.searchForm.faburenxingming){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`faburen`, params);
                } else {
                    res = await this.$api.list(`faburen`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('faburen', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.faburenzhanghao){
					searchForm['faburenzhanghao'] = '%' + this.searchForm.faburenzhanghao + '%'
				}
				if(this.searchForm.faburenxingming){
					searchForm['faburenxingming'] = '%' + this.searchForm.faburenxingming + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`faburen`, searchForm);
                } else {
                    res = await this.$api.list(`faburen`, searchForm);
                }
				// 如果是第一页数据置空
